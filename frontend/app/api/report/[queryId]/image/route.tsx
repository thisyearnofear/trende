...
export async function GET(
  ...
  const API_BASE = process.env.NEXT_PUBLIC_API_URL || process.env.INTERNAL_API_URL || 'http://localhost:8000';
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.error(`Report image data fetch failed: ${response.status} ${response.statusText}`);
    }
  } catch (e) {
  ...
  if (!data || !data.summary) {
    try {
      return new ImageResponse(
        (
          <div style={{
            ...
            <div style={{ fontSize: 60, fontWeight: 900, marginBottom: 20 }}>TRENDE</div>
      ...
      return new Response('Report image rendering failed', { status: 500 });
    }
  }

  const { summary, query, weightedConfidence, sourceCount } = data;
  const briefOverview = (summary.overview || '').slice(0, 220);
  const idea = (query.idea || 'Trend report').slice(0, 90);
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '36px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
              ...
              <div style={{ fontSize: '16px', color: '#fff', fontWeight: 700 }}>{queryId.slice(0, 12)}...</div>
            </div>
          </div>

          <div style={{ fontSize: '44px', fontWeight: 900, marginBottom: '22px', textTransform: 'uppercase', lineHeight: 1.1 }}>
            {idea}
          </div>
          ...
          <div style={{ display: 'flex', gap: '20px', marginBottom: '28px' }}>
            <div style={{ flex: 1, backgroundColor: '#141414', border: '2px solid #fff', padding: '20px', display: 'flex', flexDirection: 'column' }}>
              <div style={{ fontSize: '14px', color: '#666', marginBottom: '8px' }}>CONFIDENCE</div>
              <div style={{ fontSize: '36px', fontWeight: 900, color: '#00ff88' }}>{weightedConfidence}%</div>
            </div>
            <div style={{ flex: 1, backgroundColor: '#141414', border: '2px solid #fff', padding: '20px', display: 'flex', flexDirection: 'column' }}>
              <div style={{ fontSize: '14px', color: '#666', marginBottom: '8px' }}>SIGNALS</div>
              <div style={{ fontSize: '36px', fontWeight: 900, color: '#00ffff' }}>{sourceCount}</div>
            </div>
            <div style={{ flex: 1, backgroundColor: '#141414', border: '2px solid #fff', padding: '20px', display: 'flex', flexDirection: 'column' }}>
              <div style={{ fontSize: '14px', color: '#666', marginBottom: '8px' }}>SENTIMENT</div>
              <div style={{ fontSize: '36px', fontWeight: 900, color: '#ffaa00', textTransform: 'uppercase' }}>{summary.sentiment}</div>
            </div>
          </div>
